...
An **Order Management** System in an e-commerce context, made of two
Components: an **Order Service** that takes orders, and an **Order
Fulfillment Service** that ships them. Drawn as the default one-page
diagram of Section 3.2 — Order Service opened, every other Component
closed, including the one that shares its System:
...
graph LR
    ...
    Fulfillment(Component: Order Fulfillment Service) -->|consumes| OrderPlaced
    Fulfillment -->|references| OrderData
    Fulfillment -->|owns| Shipment[(Data Object: Shipment)]
    Fulfillment -->|produces| OrderShipped{{Event: Order Shipped}}
...
`Order Placed`, which three Components consume.
 
The System boundary changes nothing about how the Components talk. Order
Fulfillment Service is in the same System as Order Service, but it is a
separate lifecycle, so it reacts to `Order Placed` the same way Billing
and Inventory do — an Event crosses a Component boundary whether or not
it also crosses a System boundary (principle 3). It `references` the
Order data it needs from the Event's payload, owns its own `Shipment`
Data Object, and announces `Order Shipped` in turn. Because the System
groups it, a stakeholder asking "what does Order Management do?" gets
both Components' Functions as the answer; because it is closed on this
diagram, its own Functions belong on its own page.
 
Billing `references` the Order data through the Order Management API
rather than reading it directly (principle 4) — which is why Billing
also `depends-on` Order Service. Fulfillment and Inventory consume the
Event but draw no `depends-on` edge: the Event is the coupling.
...
`Order Placed`. At Context level, Order Service `produces` it and three
Components `consume` it — that's the whole story on the diagram above.
...
diagram never had to show. Fulfillment's side would read the same way —
some Function such as `Create Shipment` consuming `Order Placed`,
modifying `Shipment`, producing `Order Shipped` — and belongs on
Fulfillment's own page.
